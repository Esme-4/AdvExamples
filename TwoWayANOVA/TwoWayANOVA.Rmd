...
A more complex model for analysis of variance is the case that we can relate two classes of predictor variables. The two-way ANOVA problems have two different classes of predictor variables *V1* and *V2* and therefore they can have interactions between the predictor variables, in R this is written like $Y ~ V1 * V2$ which is equivalent to $Y ~ V1 + V2 + V1:V2$.    
...
```{r}
HRV_aov <- aov(AMP ~ factor(VEG) * factor(SMOKE), data = HRV_new)
summary(HRV_aov)

HRV_lm <- lm(AMP ~ factor(VEG) * factor(SMOKE), data = HRV_new)
summary(HRV_lm)
```

The interaction term is where we can find a difference. For the Ve2 and SMOKE2
