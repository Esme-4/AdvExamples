...
```
## More stuff  
More examples of different libraries, with advanced and not parametric capacity functions.The function `aov_car` is the modified verdsion not the CAR version.

```{r}
# Using afex library
library(afex)

ps_chronPain_aovCar <- aov_car(APhFq ~ Treatment + Error(Subject/Treatmet), data= chronPain_long)
ps_chronPain_aovCar

ps_chronPain_aovCar_phoc <- emmeans(ps_chronPain_aovCar, ~Treatment)

#For nonparametric
nonP_chronPain <- chronPain_long %>%
  friedman_test(APhFq ~ Treatment|Subject)
